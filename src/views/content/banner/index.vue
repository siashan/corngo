<template>
  ...
  <el-button type="danger" icon="el-icon-delete" @click="_delBanner">删除</el-button>
</el-button-group>
    ...
    <el-table-column
      label="图片">
       <template slot-scope="scope">
         <img :src="showImg(scope.row.imgPath)"   />
        </template>
    </el-table-column>
    <el-table-column
      label="标题"
      ...
      <el-form :rules="rules" ref="dataForm" :model="temp" label-position="left" label-width="70px" style='width: 90%; margin-left:50px;'>
        <el-row>
          ...
          <el-col :span="11" :offset="1">
             <el-form-item label="链接地址" prop="imgSrc">
           <el-input v-model="temp.imgSrc"></el-input>
        </el-form-item>
          </el-col>
        </el-row>
         <el-row>
          ...
          <el-col :span="11" :offset="1">
             <el-form-item label="展示位置" prop="showPlat">
           <el-select v-model="temp.showPlat" placeholder="请选择">
            <el-option
        ...
        <el-upload
          ...
          <img v-if="temp.imgPath" :src="showImg(temp.imgPath)" class="avatar">
...
import { getBannerList, delBanner, saveBanner } from '@/api/admin/banner'
...
export default {
  ...
  methods: {
    ...
    fmtPlat(row, column, cellValue, index) {
      if (row.status === '1') {
        return 'PC端'
      } else {
        return '移动端'
    ...
    _delBanner() {
      if (this.currentRow === null) {
        ...
        MessageBox.confirm('您确定要删除该记录吗', '提示', {
          ...
          delBanner(this.currentRow.id).then(res => {
            Message({
              message: res.msg,
              type: 'success',
              duration: 3 * 1000
