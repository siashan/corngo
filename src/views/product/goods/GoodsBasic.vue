  ...
  <el-form-item  label="商品描述" prop="content">
   <u-meditor v-model="form.goodsIntro"></u-meditor>
  </el-form-item>
  <el-form-item>
...
import { loadCat, loadBrand, saveBasic, getGoods } from '@/api/product/goods'
...
export default {
  ...
  methods: {
    ...
    _loadBrand(catId) {
      ...
      const id = catId[len - 1]
      loadBrand(id).then((res) => {
        this.brandOptions = res.data
      })
    },
    _loadBrandByCatId(catId) {
      loadBrand(catId).then((res) => {
        this.brandOptions = res.data
      })
    },
    handleClose() {
